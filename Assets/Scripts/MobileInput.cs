...
void HandleTouches()
{
    ...
    foreach (Touch touch in Input.touches)
    {
        ...
        if (touch.phase == TouchPhase.Began && touch.position.x < Screen.width * leftSideLimit && leftFingerId == -1)
        {
            ...
            //if something has been hit and its the left side
            //then left fingerid will be the touch finger id
            if (hit != null && hit.transform == leftHalf)
            {
                leftFingerId = touch.fingerId;

                //play cookie break sfx when the left side is touched
            ...
            rightFingerId == -1)
        {
            ...
            Collider2D hit =
                Physics2D.OverlapPoint(
                    world,
                    cookieDrag.draggableLayer
                );

            //if something has been hit and its the right side
            //then right fingerid will be the touch finger id
            //then play cookiedrag sfx
            if (hit != null && hit.transform == rightHalf)
            {
                rightFingerId = touch.fingerId;

                //play cookie break sfx when the right side is touched
                if (cookieDrag.cookieBreakSFX != null)
                    cookieDrag.cookieBreakSFX.Play();
            }
        }
...
void MoveLeftHalf()
{
    ...
    Vector3 local =
        leftHalf.parent.InverseTransformPoint(world);

    float offset =
        Mathf.Clamp(
            local.x - leftStartPos.x,
            -leftMoveLimit,
            0f
        );
...
void MoveRightHalf()
{
    ...
    Vector3 local =
        rightHalf.parent.InverseTransformPoint(world);

    float offset =
        Mathf.Clamp(
            local.x - rightStartPos.x,
            0f,
            rightMoveLimit
        );
...
{
    float gap =
        rightHalf.localPosition.x -
        leftHalf.localPosition.x;
